<script lang="ts">
     ...
     * 
     */   
    var collection_ingredients=[];
    let load=true;
    onMount(async () => {
        api.get('/ingredients').then(data=>{
            if(typeof data.success !== 'undefined'
            && data.success!=false){
                collection_ingredients=data.data;
            }
            load=false;
        }).catch(()=>{
            load=false;
    ...
    {:else}
        <div class="grid grid-flow-row gap-8 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 px-4 md:px-8 lg:px-0">
            {#each collection_ingredients as data}
                <Card class="max-w-sm rounded overflow-hidden shadow-lg bg-white flex justify-center">  
                    <div class="flex justify-center items-center justify-items-center p-4">
                        <img class="object-none object-center object-scale-down w-3/6" src="imgs/png/{data.ingredient}.png" alt="{data.ingredient}">
                    </div>
                    <div class="px-6 py-4 ">
                        <div class="font-bold text-xl mb-2">{data.ingredient}</div>
                        <p class="text-gray-700 text-base">{data.qty}                        </p>
                    </div>
                </Card> 
            {:else}
                <div class="col-span-1 sm:col-span-2 md:col-span-3 lg:col-span-3">            
                    <Alert on:click={cerrar_alerta} opciones={{titulo:'¡Ups! parece que ocurrio un error',mensaje:'Parece que hubo un error al cargar la información',tipo:'error',tiempo:0}}/>
                </div> 
            {/each}  
        </div>  
    {/if}
